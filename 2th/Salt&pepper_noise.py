...
for i in range(1, noise_img.shape[0] - 1):  # 第一列和最后一列用不到
    for j in range(1, noise_img.shape[1] - 1):  # 第一行和最后一行用不到
        ...
        mean_img[i][j] = tmp / 9
plt.subplot(223), plt.title('均值滤波后图像'),plt.axis('off')
...
plt.subplot(224), plt.title('中值滤波后图像'),plt.axis('off')
